...
async fn test_simple_repository_crud() {
    ...
    let exists = <NatsSimpleRepository as SimpleRepository<TestEntity>>::exists(&repo, &entity_id)
        .await
        .unwrap();
    assert!(exists);
...
async fn test_read_model_store() {
    ...
    impl ReadModel for TestReadModel {
        ...
        fn apply_event(
            &mut self,
            _event: &dyn cim_domain::DomainEvent,
    ...
    let client = async_nats::connect("nats://localhost:4222").await.unwrap();
    let store = NatsReadModelStore::new(client, "test-read-models".to_string())
        .await
        .unwrap();
    ...
    store
        .update_projection_status(TestReadModel::model_type(), ProjectionStatus::UpToDate)
        .await
        .unwrap();

    // Delete read model
    store
        .delete(TestReadModel::model_type(), &model.id)
        .await
        .unwrap();
...
async fn test_query_support() {
    ...
    // Build query
    let _query = QueryBuilder::new().limit(5).offset(2).build();
...
async fn test_aggregate_metadata() {
    ...
    assert_eq!(
        metadata.metadata.get("region").unwrap(),
