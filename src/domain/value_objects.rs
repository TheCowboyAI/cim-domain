...
impl Currency {
    ...
    pub fn new(code: impl Into<String>, exponent: u8, kind: CurrencyType) -> Self {
        Self { code: code.into(), exponent, kind }
    }
}
...
impl Money {
    ...
    pub fn amount_minor(&self) -> i128 { self.amount_minor }
    ...
    pub fn checked_add(&self, other: &Money) -> Result<Money, String> {
        ...
        Ok(Money { amount_minor: self.amount_minor + other.amount_minor, currency: self.currency.clone() })
    }
...
impl HasConcept for Money {
    fn concept_id() -> &'static str { "money" }
}
...
impl Money {
    ...
    pub fn convert_to<R: RateProvider>(&self, target: &Currency, rates: &R) -> Result<Money, String> {
        if &self.currency == target { return Ok(self.clone()); }
        let (num, den) = rates
            .latest_rate(&self.currency, target)
            .ok_or_else(|| "No conversion rate".to_string())?;
        if den == 0 { return Err("Invalid rate denominator 0".to_string()); }
        // Round to nearest minor unit with symmetric rounding
        let sign = if self.amount_minor >= 0 { 1i128 } else { -1i128 };
        let abs = self.amount_minor.abs();
        let scaled = abs.saturating_mul(num);
        let rounded = (scaled + (den / 2)) / den;
        let minor = sign * rounded;
        Ok(Money { amount_minor: minor, currency: target.clone() })
    }
    ...
    pub fn add_in_currency<R: RateProvider>(
        ...
        Ok(Money { amount_minor: a.amount_minor + b.amount_minor, currency: target.clone() })
    }
}
...
impl PhysicalAddress {
    ...
    pub fn new(
        ...
        Self { street, locality, region, subregion: None, country, postal_code }
    }
    ...
    /// City or locality
    pub fn locality(&self) -> &str { &self.locality }
    /// Region or state/province
    pub fn region(&self) -> &str { &self.region }
    /// Optional subregion
    pub fn subregion(&self) -> Option<&str> { self.subregion.as_deref() }
    /// Country code
    ...
    pub fn with_street(mut self, street: String) -> Self { self.street = street; self }
    /// Return a copy with new locality
    pub fn with_locality(mut self, locality: String) -> Self { self.locality = locality; self }
    /// Return a copy with new region
    pub fn with_region(mut self, region: String) -> Self { self.region = region; self }
    /// Return a copy with new country
    pub fn with_country(mut self, country: String) -> Self { self.country = country; self }
    /// Return a copy with new postal code
    pub fn with_postal_code(mut self, code: String) -> Self { self.postal_code = code; self }
}
...
impl Temperature {
    ...
    pub fn new(value: f64, scale: TemperatureScale) -> Self {
        ...
        Self { micro_kelvin: (kelvin * 1_000_000.0).round() as i64 }
    }
...
mod tests {
    ...
    fn money_is_value_object_and_enforces_currency() {
        ...
        let btc = Currency::new("BTC", 8, CurrencyType::Crypto { chain: "bitcoin".into() });
    ...
    fn money_exponent_behavior_for_jpy_and_btc() {
        ...
        let jpy = Currency::new("JPY", 0, CurrencyType::Fiat { country: "JP".into() });
    ...
    fn money_serde_roundtrip() {
        let eur = Currency::new("EUR", 2, CurrencyType::Fiat { country: "EU".into() });
    ...
    fn money_equality_depends_on_currency() {
        let pts_other = Currency::new("PTS", 2, CurrencyType::Other { description: "points".into() });
        let pts_chain = Currency::new("PTS", 2, CurrencyType::Crypto { chain: "sidechain".into() });
    ...
    #[derive(Default)]
    struct StaticRates { entries: Vec<ConversionRate> }
    ...
    fn money_conversion_and_addition_in_target_currency() {
        ...
        let usd = Currency::new("USD", 2, CurrencyType::Fiat { country: "US".into() });
        let gbp = Currency::new("GBP", 2, CurrencyType::Fiat { country: "GB".into() });
        let mut rates = StaticRates::default();
        rates.entries.push(ConversionRate { from: gbp.clone(), to: usd.clone(), num: 120, den: 100, at: 1000 });
