...
impl SemanticAnalyzer {
    ...
    pub async fn find_similar(
        ...
        let target = concepts
            ...
            .ok_or_else(|| DomainError::NotFound(format!("Concept {concept_name} not found")))?;
        ...
        for concept in concepts.iter() {
            if concept.name != concept_name {
                let similarity =
                    cosine_similarity(&target.quality_dimensions, &concept.quality_dimensions);
    ...
    pub async fn semantic_distance(
        ...
        let embedding_a = embeddings
            .get(concept_a)
            .ok_or_else(|| DomainError::NotFound(format!("Concept {concept_a} not found")))?;

        let embedding_b = embeddings
            .get(concept_b)
            .ok_or_else(|| DomainError::NotFound(format!("Concept {concept_b} not found")))?;
    ...
    pub async fn find_alignments(
        ...
        let source_concepts: Vec<_> = embeddings
            .values()
            .filter(|e| e.domain == source_domain)
            .collect();

        let target_concepts: Vec<_> = embeddings
            .values()
    ...
    fn ontology_distance(&self, _domain: &str, concept_a: &str, concept_b: &str) -> Option<u32> {
...
impl SemanticAnalyzer {
    ...
    pub async fn embed_composition(
        &self,
...
mod tests {
    ...
    async fn test_semantic_distance() {
        ...
        // Add test embeddings
        analyzer
            .add_embedding(ConceptEmbedding {
        ...
        // Test distances
        let dist_ab = analyzer
            .semantic_distance("concept_a", "concept_b")
            .await
            .unwrap();
        let dist_ac = analyzer
            .semantic_distance("concept_a", "concept_c")
            .await
            .unwrap();
