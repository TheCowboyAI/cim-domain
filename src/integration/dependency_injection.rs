...
type ProviderMap = HashMap<TypeId, Box<dyn Any + Send + Sync>>;
type FactoryFn<T> = Box<dyn Fn(&DependencyContainer) -> Result<Arc<T>, DomainError> + Send + Sync>;
...
pub trait ServiceProvider: Send + Sync {
    ...
    ) -> Result<Arc<Self::Service>, DomainError>;
}
...
impl DependencyContainer {
    ...
    pub async fn register_instance<T: Injectable + 'static>(
        &self,
        service: T,
    ) -> Result<(), DomainError> {
        ...
        P::Service: 'static,
    {
        ...
        if providers.contains_key(&type_id) {
            return Err(DomainError::AlreadyExists(format!(
                "Provider for type {} already registered",
                std::any::type_name::<P::Service>()
            )));
        }
        ...
        Fut: std::future::Future<Output = Result<Arc<T>, DomainError>> + Send + 'static,
    {
        ...
        Err(DomainError::NotImplemented(
            "Async factories not yet supported".to_string(),
        ))
    }
    ...
    where
        ...
        F: Fn(&DependencyContainer) -> Result<Arc<T>, DomainError> + Send + Sync + 'static,
    {
        self.register_provider(SingletonProvider::new(factory))
            .await
    }
...
impl ContainerBuilder {
    ...
    pub async fn add_instance<T: Injectable + 'static>(
        self,
        service: T,
...
mod tests {
    ...
    async fn test_dependency_resolution() {
        ...
        container
            .register_instance(DependentService {
    ...
    async fn test_async_factory_not_supported() {
        ...
        let result = container
            .register_async_factory(|_| async {
                Ok(Arc::new(TestService {
    ...
    async fn test_domain_category_ext() {
        ...
        let category = DomainCategory::create_with_di("TestCategory".to_string(), &container)
            .await
            .unwrap();
    ...
    async fn test_factory_error_propagation() {
        ...
        container
            .register_factory(|_| -> Result<Arc<TestService>, DomainError> {
