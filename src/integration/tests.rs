...
mod integration_tests {
    use crate::{
        ...
        events::DomainEvent,
        integration::*,
        composition::{Saga, SagaState, SagaStep, RetryPolicy},
    ...
    impl AggregateEventHandler for CollectingEventHandler {
        async fn handle_event(&self, event: &Box<dyn DomainEvent>) -> Result<(), DomainError> {
            ...
            let test_event = TestDomainEvent {
                ...
                aggregate_type: event.subject().split('.').next().unwrap_or("Unknown").to_string(),
    ...
    impl CommandBus for TestCommandBus {
        async fn send(&self, command: Box<dyn SagaCommand>) -> Result<(), String> {
            ...
            commands.push(format!("{} for {}", command.command_type(), command.aggregate_id()));
            Ok(())
        }
    }
    ...
    async fn test_aggregate_event_routing_integration() {
        ...
        router.register_handler("Person", Box::new(person_handler)).await.unwrap();
        router.register_handler("Organization", Box::new(org_handler)).await.unwrap();
    ...
    async fn test_domain_bridge_translation() {
        ...
        let result = registry.send_command("Person", "HR", command, context).await;
        ...
        assert_eq!(translated.payload.get("work_email").unwrap(), "john@example.com");
    }
    ...
    async fn test_saga_coordination_with_event_routing() {
        ...
        coordinator.handle_event(&step1_event, Some(&saga_id)).await.unwrap();
    ...
    async fn test_cross_domain_search_integration() {
        ...
        impl DomainSearcher for PersonSearcher {
            ...
            async fn get_relationships(
                ...
                Ok(vec![
                    ("Person".to_string(), "works_for".to_string(), "Organization".to_string()),
                ])
            }
        }
    ...
    async fn test_full_integration_flow() {
        ...
        let routed = event_router.route_event("Person", &person_event).await.unwrap();
