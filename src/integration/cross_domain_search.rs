...
type AnalyzerMap = HashMap<String, Arc<SemanticAnalyzer>>;
type DomainMap = HashMap<String, DomainCategory>;
type FunctorMap = HashMap<(String, String), Box<dyn std::any::Any + Send + Sync>>;
type SearcherMap = HashMap<String, Box<dyn DomainSearcher>>;
...
pub struct CrossDomainSearchEngine {
    ...
    analyzers: Arc<RwLock<AnalyzerMap>>,

    /// Domain categories
    domains: Arc<RwLock<DomainMap>>,

    /// Functors between domains for concept mapping
    functors: Arc<RwLock<FunctorMap>>,
...
impl CrossDomainSearchEngine {
        ...
        F: DomainFunctor + 'static,
    {
        ...
        if functors.contains_key(&key) {
            return Err(DomainError::AlreadyExists(format!(
                "Functor from {} to {} already exists",
                key.0, key.1
            )));
        }
    ...
    pub async fn search_advanced(
    ...
    ) -> Result<CrossDomainResult, DomainError> {
        ...
        let concept_vector = match query.concept_vector.clone() {
            Some(v) => v,
            None => {
                self.compute_query_vector(&query.query, query.start_domain.as_deref())
                    .await?
            }
        };
        ...
        for domain in &domains_to_search {
            let results = self.search_domain(domain, &concept_vector, config).await?;

            // Find cross-domain links
            if config.follow_relationships {
                for result in &results {
                    let links = self
                        .find_cross_domain_links(domain, &result.concept_name, config.max_depth)
                        .await?;
                    ...
                    for link in &links {
                        all_relationships.push(DomainRelationship {
                            ...
                            shared_concepts: vec![
                                link.source_concept.clone(),
                                link.target_concept.clone(),
                            ],
        ...
        let truncated = domain_results
            .values()
            .any(|results| results.len() >= config.results_per_domain);
    ...
    async fn get_domains_to_search(
        &self,
        query: &CrossDomainQuery,
    ...
    async fn compute_query_vector(
        ...
        if let Some(domain) = start_domain {
            ...
            if let Some(analyzer) = analyzers.get(domain) {
                ...
                let concept = analyzer
                    .create_concept_from_text(query)
                    .await
    ...
    async fn search_domain(
        ...
        let analyzer = analyzers
            .get(domain)
            .ok_or_else(|| DomainError::NotFound(format!("Analyzer for {domain} not found")))?;
        ...
        let similar = analyzer
            .find_similar("_search_", config.min_similarity)
            .await?;
                    ...
                    if let Some(target_analyzer) = analyzers.get(to_domain) {
                        ...
                        if let Some(source_analyzer) = analyzers.get(&current_domain) {
                            if let Ok(source_concepts) = source_analyzer.get_concepts().await {
                                if let Some(source_concept) =
                                    source_concepts.iter().find(|c| c.name == current_concept)
                                {
                                    // Find similar concepts in target domain
                                    if let Ok(similar) = target_analyzer
                                        .find_similar(&source_concept.name, 0.6)
                                        .await
                                    {
                                        for (target_concept_name, similarity) in similar {
                                            ...
                                            let key =
                                                (to_domain.clone(), target_concept_name.clone());
    ...
    async fn aggregate_concepts(
            ...
            if let Some(analyzer) = analyzers.get(domain) {
                for result in results {
                    ...
                    if let Ok(concepts) = analyzer.get_concepts().await {
                        if let Some(concept) =
                            concepts.iter().find(|c| c.name == result.concept_name)
                        {
                            all_concepts.push((
                                domain.clone(),
        ...
        for (domain, concept_name, similarity, vector) in all_concepts {
            ...
            for (group_key, group) in concept_groups.iter() {
                ...
                if let Some((first_domain, _)) =
                    group.domains.first().zip(group.variations.iter().next())
                {
                    if let Some(analyzer) = analyzers.get(first_domain) {
                        if let Ok(concepts) = analyzer.get_concepts().await {
                            if let Some(group_concept) = concepts.iter().find(|c| {
                                group
                                    .variations
                                    .get(first_domain)
                                ...
                                let sim =
                                    cosine_similarity(&vector, &group_concept.quality_dimensions);
            ...
            if let Some(group_key) = best_group {
                ...
                concept_groups.entry(group_key).and_modify(|agg| {
                ...
                let key = format!(
                    "{}_{}",
                    concept_name.to_lowercase().replace(' ', "_"),
                    concept_groups.len()
                );
                concept_groups.insert(
                    key,
                    AggregatedConcept {
        ...
        for ((domain_a, domain_b), rels) in grouped {
            ...
            let merged = DomainRelationship {
                ...
                relationship_type: if all_types.len() == 1 {
                    ...
                    format!(
                        "multi[{}]",
                        all_types.into_iter().collect::<Vec<_>>().join(",")
                    )
                },
...
mod tests {
    ...
    async fn test_cross_domain_search() {
        ...
        engine
            .register_domain(sales_domain, sales_analyzer.clone())
            .await
            .unwrap();
        ...
        engine
            .register_domain(billing_domain, billing_analyzer.clone())
            .await
            .unwrap();
    ...
    async fn test_register_domain_duplicate() {
        ...
        assert!(engine
            .register_domain(domain.clone(), analyzer.clone())
            .await
            .is_ok());
    ...
    async fn test_compute_query_vector() {
        ...
        let vector = engine
            .compute_query_vector("test query", None)
            .await
            .unwrap();
        ...
        let vector = engine
            .compute_query_vector("test query", Some("TestDomain"))
            .await
            .unwrap();
    ...
    async fn test_deduplicate_relationships() {
        ...
        let ab_rel = deduplicated
            .iter()
            .find(|r| {
                (r.domain_a == "A" && r.domain_b == "B") || (r.domain_a == "B" && r.domain_b == "A")
            })
            .unwrap();
