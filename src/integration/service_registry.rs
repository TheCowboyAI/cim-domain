...
type ServiceFactoryFn = dyn Fn(&DependencyContainer) -> Result<Box<dyn Any + Send + Sync>, DomainError>
    + Send
    + Sync;
type ServiceDescriptorMap = HashMap<TypeId, ServiceDescriptor>;
type InstanceMap = HashMap<TypeId, Box<dyn Any + Send + Sync>>;
type TagToServicesMap = HashMap<String, Vec<TypeId>>;
type EndpointMap = HashMap<TypeId, ServiceEndpoint>;
...
pub struct ServiceDescriptor {
    ...
    /// Factory function
    pub factory: Arc<ServiceFactoryFn>,
...
pub struct ServiceRegistry {
    /// Registered services
    services: Arc<RwLock<ServiceDescriptorMap>>,
...
pub struct ServiceDiscovery {
    ...
    services_by_tag: TagToServicesMap,
...
impl ServiceRegistry {
    ...
    where
        ...
        F: Fn(&DependencyContainer) -> Result<Box<TImpl>, DomainError> + Send + Sync + 'static,
    {
        self.register::<TService, TImpl, F>(ServiceLifetime::Singleton, factory)
            .await
    }
    ...
    where
        ...
        F: Fn(&DependencyContainer) -> Result<Box<TImpl>, DomainError> + Send + Sync + 'static,
    {
        self.register::<TService, TImpl, F>(ServiceLifetime::Transient, factory)
            .await
    }

    /// Register a scoped service
    pub async fn register_scoped<TService, TImpl, F>(&self, factory: F) -> Result<(), DomainError>
    where
        TService: ?Sized + 'static,
        TImpl: 'static + Send + Sync,
        F: Fn(&DependencyContainer) -> Result<Box<TImpl>, DomainError> + Send + Sync + 'static,
    {
        self.register::<TService, TImpl, F>(ServiceLifetime::Scoped, factory)
            .await
    }
        ...
        match descriptor.lifetime {
            ServiceLifetime::Singleton => {
                ...
                if let Ok(boxed_t) = boxed_instance.downcast::<T>() {
                    ...
                    instances.insert(
                        service_type,
                        Box::new(instance.clone()) as Box<dyn Any + Send + Sync>,
                    );
...
impl ServiceCollectionBuilder {
    ...
    pub async fn add_singleton<TService, TImpl, F>(self, factory: F) -> Result<Self, DomainError>
    where
        TService: ?Sized + 'static,
        TImpl: 'static + Send + Sync,
        F: Fn(&DependencyContainer) -> Result<Box<TImpl>, DomainError> + Send + Sync + 'static,
    {
        self.registry
            .register_singleton::<TService, TImpl, F>(factory)
            .await?;
        Ok(self)
    }
    ...
    /// # Arguments
    /// * `factory` - Factory function to create the service
    pub async fn add_transient<TService, TImpl, F>(self, factory: F) -> Result<Self, DomainError>
    where
        TService: ?Sized + 'static,
        TImpl: 'static + Send + Sync,
        F: Fn(&DependencyContainer) -> Result<Box<TImpl>, DomainError> + Send + Sync + 'static,
    {
        self.registry
            .register_transient::<TService, TImpl, F>(factory)
            .await?;
        Ok(self)
    }
    ...
    /// # Arguments
    /// * `factory` - Factory function to create the service
    pub async fn add_scoped<TService, TImpl, F>(self, factory: F) -> Result<Self, DomainError>
    where
        TService: ?Sized + 'static,
        TImpl: 'static + Send + Sync,
        F: Fn(&DependencyContainer) -> Result<Box<TImpl>, DomainError> + Send + Sync + 'static,
    {
        self.registry
            .register_scoped::<TService, TImpl, F>(factory)
            .await?;
        Ok(self)
    }
...
mod tests {
    ...
    async fn test_service_registration() {
        ...
        // Register singleton
        registry
            .register::<TestServiceImpl, TestServiceImpl, _>(ServiceLifetime::Singleton, |_| {
        ...
        // Resolve service
        let service1 = registry
            .resolve::<TestServiceImpl>(&container)
            .await
            .unwrap();
        let service2 = registry
            .resolve::<TestServiceImpl>(&container)
            .await
            .unwrap();
    ...
    async fn test_transient_lifetime() {
        ...
        // Register transient
        registry
            .register::<TestServiceImpl, TestServiceImpl, _>(
    ...
    async fn test_service_discovery() {
        ...
        // Register service
        registry
            .register_singleton::<dyn TestService, TestServiceImpl, _>(|_| {
                Ok(Box::new(TestServiceImpl {
    ...
    async fn test_service_endpoints() {
        ...
        registry
            .register_endpoint::<dyn TestService>(endpoint.clone())
            .await
            .unwrap();
    ...
    async fn test_service_collection_builder() {
        let registry = ServiceCollectionBuilder::new()
            ...
            .await
            .unwrap()
            .with_tags::<dyn TestService>(vec!["built-service".to_string()])
            .await
            .unwrap()
            .build();
