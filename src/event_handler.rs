...
mod tests {
    ...
    impl EventHandler<TestEvent> for ValidatingEventHandler {
        ...
        async fn handle(&self, event: TestEvent) -> Result<(), Self::Error> {
            if event.value < self.min_value {
                Err(TestError::ValidationError(format!(
                    "Value {} is less than minimum {}",
                    event.value, self.min_value
                )))
            } else {
                Ok(())
            }
    ...
    impl EventHandler<TestEvent> for ProcessingEventHandler {
        ...
        async fn handle(&self, mut event: TestEvent) -> Result<(), Self::Error> {
            ...
            if event.value > 1000 {
                return Err(TestError::ProcessingError(format!(
                    "Value {} is too large to process",
                    event.value
                )));
            }
    ...
    async fn test_composite_event_handler() {
        ...
        let composite = CompositeEventHandler {
            handlers: vec![Box::new(counting_handler), Box::new(processing_handler)],
        };
    ...
    async fn test_error_propagation() {
        ...
        let composite = CompositeEventHandler {
            handlers: vec![Box::new(validating_handler), Box::new(counting_handler)],
        };
    ...
    async fn test_different_event_type() {
        ...
        assert!(handler
            .handle(StringEvent("hello".to_string()))
            .await
            .is_ok());
