...
pub trait CheckpointStore: Send + Sync {
    ...
    async fn save_checkpoint(&self, checkpoint: &ProjectionCheckpoint) -> Result<(), CheckpointError>;
    
    /// Load a checkpoint by projection ID
    async fn load_checkpoint(&self, projection_id: &str) -> Result<Option<ProjectionCheckpoint>, CheckpointError>;
...
impl JetStreamCheckpointStore {
    ...
    pub async fn new(client: async_nats::Client, bucket_name: &str) -> Result<Self, CheckpointError> {
        ...
        let kv_store = jetstream
            ...
            .await
            .map_err(|e| CheckpointError::Nats(async_nats::Error::from(std::io::Error::new(
...
impl CheckpointStore for JetStreamCheckpointStore {
    ...
    async fn load_checkpoint(&self, projection_id: &str) -> Result<Option<ProjectionCheckpoint>, CheckpointError> {
        ...
        match self.kv_store.get(&key).await {
            ...
            Ok(None) => Ok(None),
            Err(e) => Err(CheckpointError::Nats(async_nats::Error::from(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string()
            )))),
        }
    ...
    async fn list_checkpoints(&self) -> Result<Vec<ProjectionCheckpoint>, CheckpointError> {
        ...
        let mut entries = self.kv_store.watch("checkpoint:*").await
            .map_err(|e| CheckpointError::Nats(async_nats::Error::from(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string()
            ))))?;
        ...
        while let Some(entry) = entries.next().await {
            match entry {
                Ok(entry) => {
                    if let Ok(checkpoint) = serde_json::from_slice::<ProjectionCheckpoint>(&entry.value) {
...
impl CheckpointManager {
    ...
    pub async fn get_position(&self, projection_id: &str) -> Result<Option<EventPosition>, CheckpointError> {
        Ok(self.store
            .load_checkpoint(projection_id)
            .await?
            .map(|c| c.position))
...
mod tests {
    ...
    async fn test_checkpoint_operations() {
        ...
        manager.record_error(projection_id, "Test error").await.unwrap();
