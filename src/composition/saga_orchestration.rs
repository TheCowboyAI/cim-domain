...
use crate::state_machine::{State, MealyStateTransitions, MealyMachine, TransitionInput, TransitionOutput};
    ...
    fn description(&self) -> String {
        match self {
            ...
            SagaTransitionInput::CompensationCompleted { step_id } => {
                format!("Compensation for {} completed", step_id)
            }
            SagaTransitionInput::CompensationFailed { step_id, error } => {
                format!("Compensation for {} failed: {}", step_id, error)
            }
        }
    }
}
...
impl MealyStateTransitions for SagaState {
    ...
    fn can_transition_to(&self, target: &Self, input: &Self::Input) -> bool {
        match (self, target, input) {
            ...
            (SagaState::Running { .. }, SagaState::Compensating { .. }, SagaTransitionInput::StepFailed { .. }) => true,
            
            // Compensating -> Compensating
            (SagaState::Compensating { .. }, SagaState::Compensating { .. }, SagaTransitionInput::CompensationCompleted { .. }) => true,
            
            // Compensating -> Compensated
            (SagaState::Compensating { .. }, SagaState::Compensated, SagaTransitionInput::CompensationCompleted { .. }) => true,
            
            // Compensating -> Failed
            (SagaState::Compensating { .. }, SagaState::Failed { .. }, SagaTransitionInput::CompensationFailed { .. }) => true,
    ...
    fn valid_transitions(&self, input: &Self::Input) -> Vec<Self> {
        match (self, input) {
            ...
            (SagaState::Running { completed_steps, .. }, SagaTransitionInput::StepCompleted { step_id, .. }) => {
    ...
    fn transition_output(&self, target: &Self, input: &Self::Input) -> Self::Output {
        ...
        match (self, target, input) {
            ...
            (SagaState::Running { .. }, SagaState::Completed, SagaTransitionInput::StepCompleted { .. }) => {
                events.push(SagaEvent::Completed {
                    saga_id: Uuid::new_v4(),
                    timestamp: chrono::Utc::now(),
                });
            }
            (SagaState::Running { .. }, SagaState::Compensating { .. }, SagaTransitionInput::StepFailed { step_id, error }) => {
...
pub struct SagaOrchestrator {
    ...
    sagas: Arc<Mutex<HashMap<Uuid, (Saga, MealyMachine<SagaState, SagaAggregate>)>>>,
...
impl SagaOrchestrator {
    ...
    pub async fn register_domain(&self, domain: DomainCategory) -> Result<(), DomainError> {
        ...
        if domains.contains_key(&domain.name) {
            return Err(DomainError::AlreadyExists(
                format!("Domain {} already registered", domain.name)
            ));
        }
    ...
    pub async fn start_saga(&self, mut saga: Saga) -> Result<Uuid, DomainError> {
        ...
        self.transition_saga(saga_id, SagaTransitionInput::Start).await?;
    ...
    fn transition_saga(
        ...
        Box::pin(async move {
        let mut sagas = self.sagas.lock().await;
        let (saga, state_machine) = sagas.get_mut(&saga_id)
            .ok_or_else(|| DomainError::NotFound(format!("Saga {} not found", saga_id)))?;
        
        // Determine target state based on current state and input
        let target_state = match (state_machine.current_state(), &input) {
            (SagaState::Pending, SagaTransitionInput::Start) => {
                // Find first step
                let first_step = saga.steps.iter()
                    .find(|s| s.depends_on.is_empty())
                    .ok_or_else(|| DomainError::InvalidOperation { reason: "No initial step found".to_string() })?;
            ...
            (SagaState::Compensating { compensated_steps, failed_step }, SagaTransitionInput::CompensationCompleted { step_id }) => {
            ...
            (SagaState::Compensating { .. }, SagaTransitionInput::CompensationFailed { error, .. }) => {
                SagaState::Failed {
                    error: error.clone(),
                }
            }
            _ => return Err(DomainError::InvalidOperation { reason: "Invalid state transition".to_string() }),
        };
    ...
    fn get_next_step_for_saga(
        ...
        Err(DomainError::InvalidOperation { reason: "No valid next step found".to_string() })
    }
    ...
    async fn execute_saga_command(
        ...
        match command {
            SagaCommand::ExecuteStep { step_id, domain, command } => {
                // Execute step and handle result
                match self.execute_step_internal(saga_id, &step_id, &domain, &command).await {
                    Ok(result) => {
            ...
            SagaCommand::ExecuteCompensation { step_id, action } => {
                // Execute compensation
                match self.execute_compensation_internal(saga_id, &step_id, &action).await {
                    Ok(_) => {
                        self.transition_saga(
                            saga_id,
                            SagaTransitionInput::CompensationCompleted { step_id },
    ...
    pub async fn get_saga_state(&self, saga_id: Uuid) -> Result<SagaState, DomainError> {
        ...
        let (saga, _) = sagas.get(&saga_id)
            .ok_or_else(|| DomainError::NotFound(format!("Saga {} not found", saga_id)))?;
        Ok(saga.state.clone())
    }
    
    /// Get saga state machine for direct access
    pub async fn get_state_machine(&self, saga_id: Uuid) -> Result<MealyMachine<SagaState, SagaAggregate>, DomainError> {
        let sagas = self.sagas.lock().await;
        let (_, state_machine) = sagas.get(&saga_id)
            .ok_or_else(|| DomainError::NotFound(format!("Saga {} not found", saga_id)))?;
...
mod tests {
    ...
    async fn test_saga_orchestrator() {
        ...
        // Register domains
        orchestrator.register_domain(DomainCategory::new("OrderDomain".to_string())).await.unwrap();
        orchestrator.register_domain(DomainCategory::new("InventoryDomain".to_string())).await.unwrap();
        ...
        assert!(matches!(state, SagaState::Running { current_step, .. } if current_step == "step1"));
