...
mod tests {
    ...
    mod concurrent_tests {
        ...
        async fn test_concurrent_counter_increments() {
            ...
            for task_id in 0..num_tasks {
                ...
                handles.push(tokio::spawn(async move {
                    for i in 0..increments_per_task {
                        ...
                        metrics_clone.increment(&format!("counter.task_{}", task_id)).await;
            ...
            assert_eq!(metrics.get_counter("counter.periodic").await, expected_total / 10);
            
            // Each task counter should have correct count
            for task_id in 0..num_tasks {
                assert_eq!(
                    metrics.get_counter(&format!("counter.task_{}", task_id)).await,
                    increments_per_task
                );
            }
        }
        ...
        async fn test_concurrent_duration_recording() {
            ...
            for task_id in 0..num_tasks {
                ...
                handles.push(tokio::spawn(async move {
                    for i in 0..durations_per_task {
                        ...
                        metrics_clone.record_duration("operation.shared", duration).await;
                        
                        // Task-specific operations
                        metrics_clone.record_duration(
            ...
            assert_eq!(shared_stats.count, (num_tasks * durations_per_task) as usize);
        ...
        async fn test_concurrent_error_recording() {
                ...
                handles.push(tokio::spawn(async move {
                    for i in 0..50 {
                        if (task_id + i) % 3 == 0 {
                            ...
                            metrics_clone.record_error(&format!("task_{}.error", task_id)).await;
                        } else {
                            // Record success
            ...
            assert_eq!(metrics.get_error_count("operation.failing").await, expected_errors);
        ...
        async fn test_concurrent_reset() {
            ...
            for i in 0..100 {
                ...
                metrics.record_duration("op", Duration::from_millis(i)).await;
        ...
        async fn test_high_contention_scenario() {
            ...
            for task_id in 0..num_tasks {
                ...
                handles.push(tokio::spawn(async move {
                    for i in 0..100 {
                        ...
                        metrics_clone.increment(&format!("{}.attempts", op)).await;
            ...
            for op in operations {
                let attempts = summary.counters.get(&format!("{}.attempts", op)).unwrap();
                let successes = summary.counters.get(&format!("{}.count", op)).unwrap_or(&0);
