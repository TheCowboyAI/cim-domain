...
use crate::{
    entity::EntityId,
    persistence::{
        metrics::{MetricsInstrumented, MetricsTimer, PersistenceMetrics},
        SimpleAggregateMetadata, SimpleRepository,
    },
...
mod tests {
    ...
    impl SimpleRepository<TestEntity> for MockRepository {
        ...
        ) -> Result<SimpleAggregateMetadata, DomainError> {
    ...
    async fn test_instrumented_save() {
        ...
        assert_eq!(
            instrumented
                .metrics()
                .get_counter("repository.save.count")
                .await,
            1
        );
        assert_eq!(
            instrumented
                .metrics()
                .get_counter("repository.save.success")
                .await,
            1
        );
        assert_eq!(
            instrumented
                .metrics()
                .get_counter("repository.save.error")
                .await,
            0
        );
    }
    ...
    async fn test_instrumented_load() {
        ...
        assert_eq!(
            instrumented
                .metrics()
                .get_counter("repository.load.count")
                .await,
            2
        );
        assert_eq!(
            instrumented
                .metrics()
                .get_counter("repository.load.hit")
                .await,
            1
        );
        assert_eq!(
            instrumented
                .metrics()
                .get_counter("repository.load.miss")
                .await,
            1
        );
    }
