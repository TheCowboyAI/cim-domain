...
impl DomainCategory {
    ...
    pub fn define_composition(
        ...
        if first_morph.target != second_morph.source {
            return Err(DomainError::InvalidOperation {
                reason: format!("Cannot compose: {first} target != {second} source"),
            });
        }
        ...
        if result_morph.source != first_morph.source || result_morph.target != second_morph.target {
            return Err(DomainError::InvalidOperation {
                reason: "Result morphism has incorrect source/target".to_string(),
            });
        }

        self.composition_table
            .insert((first.to_string(), second.to_string()), result.to_string());
        Ok(())
    }
    ...
    pub fn compose(&self, first: &str, second: &str) -> Result<String, DomainError> {
        ...
        Err(DomainError::InvalidOperation {
            reason: format!("Composition of {first} and {second} not defined"),
        })
    }
    ...
    pub fn morphisms_of_type(&self, morph_type: &MorphismType) -> Vec<&DomainMorphism> {
        self.morphisms
            .values()
            .filter(|morph| {
                std::mem::discriminant(&morph.operation_type) == std::mem::discriminant(morph_type)
            })
            .collect()
    }
            ...
            let identity =
                self.morphisms
                    ...
                    .ok_or_else(|| DomainError::InvalidOperation {
                        reason: format!("Identity {identity_id} not found"),
                    })?;

            // Identity should have same source and target
            if identity.source != *obj_id || identity.target != *obj_id {
                return Err(DomainError::InvalidOperation {
                    reason: format!("Identity {identity_id} has wrong source/target"),
                });
            }
        }
                        ...
                        if let Ok(fg_h) = self.compose(&fg, h_id) {
                            ...
                            if let Ok(gh) = self.compose(g_id, h_id) {
                                if let Ok(f_gh) = self.compose(f_id, &gh) {
                                    ...
                                    if fg_h != f_gh {
                                        return Err(DomainError::InvalidOperation {
                                            reason: format!("Associativity violated: ({f_id} ∘ {g_id}) ∘ {h_id} != {f_id} ∘ ({g_id} ∘ {h_id})")
                                        });
                                    }
                                }
                            }
