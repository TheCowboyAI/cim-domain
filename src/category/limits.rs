...
use super::domain_category::{DomainMorphism, DomainObject, MorphismType};
...
impl Pullback {
    ...
    pub fn for_synchronization(
        ...
        let apex = DomainObject {
            id: format!("Pullback_{domain_a}_{domain_b}"),
            ...
            metadata: HashMap::from([(
                "pullback_of".to_string(),
                format!("{domain_a} and {domain_b}"),
            )]),
        };

        // Create morphisms
        let f = DomainMorphism {
            id: format!("{domain_a}_to_shared"),
...
impl Pushout {
    ...
    pub fn for_merger(
        ...
        let apex = DomainObject {
            id: format!("Merged_{domain_a}_{domain_b}"),
            composition_type: DomainCompositionType::Composite {
        ...
        // Create morphisms
        let f = DomainMorphism {
            id: format!("{common_base}_to_{domain_a}"),
        ...
        let g = DomainMorphism {
            id: format!("{common_base}_to_{domain_b}"),
            source: object_c.id.clone(),
            target: object_b.id.clone(),
            operation_type: MorphismType::Transformation {
                transform_type: "inclusion".to_string(),
    ...
    pub fn of_domains(domains: Vec<&str>) -> Result<Self, DomainError> {
        if domains.is_empty() {
            return Err(DomainError::InvalidOperation {
                reason: "Cannot create product of empty list".to_string(),
            });
        }
        ...
        for (i, domain) in domains.iter().enumerate() {
            ...
            let projection = DomainMorphism {
                ...
                metadata: HashMap::from([("component_index".to_string(), i.to_string())]),
            };
...
impl Coproduct {
    ...
    pub fn of_domains(domains: Vec<&str>) -> Result<Self, DomainError> {
        if domains.is_empty() {
            return Err(DomainError::InvalidOperation {
                reason: "Cannot create coproduct of empty list".to_string(),
            });
        }
...
mod tests {
    ...
    async fn test_pullback_creation() {
        let pullback =
            Pullback::for_synchronization("OrderDomain", "InventoryDomain", "ProductCatalog")
                .unwrap();
    ...
    async fn test_pushout_creation() {
        let pushout =
            Pushout::for_merger("SalesDomain", "MarketingDomain", "CustomerData").unwrap();
    ...
    fn test_product_creation() {
        ...
        assert_eq!(product.apex.metadata.get("arity").unwrap(), "3");
    }
