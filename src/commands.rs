...
pub struct RegisterPerson {
    ...
    pub person_id: Uuid,
    /// Identity component data
    pub identity: crate::IdentityComponent,
    /// Optional contact component
...
impl Command for RegisterPerson {
    ...
    fn aggregate_id(&self) -> Option<EntityId<Self::Aggregate>> {
        Some(EntityId::from_uuid(self.person_id))
    }
}

/// Update person's profile (add/update components)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UpdatePersonProfile {
    /// Person's ID
    pub person_id: Uuid,
    /// Components to add or update
    pub components: PersonComponentUpdates,
}

/// Components that can be updated on a person
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PersonComponentUpdates {
    /// Update identity component
    pub identity: Option<crate::IdentityComponent>,
    /// Update contact component
    pub contact: Option<crate::ContactComponent>,
    /// Update employment component
    pub employment: Option<crate::EmploymentComponent>,
    /// Update position component
    pub position: Option<crate::PositionComponent>,
    /// Update skills component
    pub skills: Option<crate::SkillsComponent>,
    /// Update access component
    pub access: Option<crate::AccessComponent>,
    /// Update external identifiers
    pub external_identifiers: Option<crate::ExternalIdentifiersComponent>,
}

impl Command for UpdatePersonProfile {
    type Aggregate = PersonAggregate;
...
mod tests {
    ...
    fn test_register_person_command() {
        ...
        let envelope = CommandEnvelope::new(cmd.clone(), "test_user".to_string());

        assert_eq!(envelope.command.person_id, cmd.person_id);
        assert_eq!(envelope.issued_by, "test_user");
        // Correlation ID should be self-reference for new commands
        match &envelope.correlation_id {
            CorrelationId(crate::IdType::Uuid(id)) => {
                assert_eq!(*id, *envelope.id.as_uuid());
            }
    ...
    fn test_command_aggregate_ids() {
        ...
        assert_eq!(agg_id.unwrap().as_uuid(), &person_id);
