...
use cim_domain::{
    ...
    integration::{
        AggregateEventHandler, AggregateEventRouter, BridgeRegistry, DependencyContainer,
        DomainBridge, ServiceLifetime, ServiceRegistry,
    },
...
impl AggregateEventHandler for UserEventHandler {
    async fn handle_event(&self, event: &Box<dyn DomainEvent>) -> Result<(), DomainError> {
        println!(
            "   [User] Handling {} event for aggregate {}",
            event.event_type(),
            event.aggregate_id()
        );
        Ok(())
...
impl AggregateEventHandler for HREventHandler {
    async fn handle_event(&self, event: &Box<dyn DomainEvent>) -> Result<(), DomainError> {
        println!(
            "   [HR] Handling {} event for aggregate {}",
            event.event_type(),
            event.aggregate_id()
        );
        Ok(())
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Register handlers
    event_router
        .register_handler("User", Box::new(UserEventHandler))
        .await?;
    event_router
        .register_handler("HR", Box::new(HREventHandler))
        .await?;
    ...
    event_router
        .register_route("User", "HR", "User.Created.*", |event| {
    ...
    // Register services
    service_registry
        .register::<EmailService, EmailService, _>(ServiceLifetime::Singleton, |_| {
            Ok(Box::new(EmailService::new()))
        })
        .await?;
    ...
    container
        .register_instance(Arc::new(EmailService::new()))
        .await?;
    ...
    config_bridge
        .metadata
        .insert("protocol".to_string(), "async".to_string());
    ...
    println!(
        "   ✓ Bridge {} → {} metadata:",
        config_runtime_bridge.source_domain, config_runtime_bridge.target_domain
    );
