...
use chrono::Utc;
use cim_domain::{persistence::*, DomainEntity, DomainError, EntityId};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(loaded_product) = loaded {
        println!(
            "  ✅ Loaded product: {} (${:.2})",
            loaded_product.name, loaded_product.price
        );
    }
    ...
    let read_model_store =
        NatsReadModelStore::new(client.clone(), "product-read-models".to_string()).await?;
    ...
    read_model_store
        .update_projection_status(ProductStats::model_type(), ProjectionStatus::UpToDate)
        .await?;
