...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let loaded_order: Order = repository.load(&order.id)
        .await?
    ...
    println!("  Version {}: Status changed to {:?}", metadata2.version, order.status);
    ...
    for i in 1..=3 {
        ...
        println!("  Order {} created (version {})", new_order.id, metadata.version);
    }
