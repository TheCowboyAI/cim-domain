...
fn classify_objects_in_domain_graph_to_core_concepts() {
    ...
    let mut money = None; let mut cmd = None; let mut event = None; let mut read_model = None;
    for o in g.category.objects { match o.name.as_str() {
        "Money" => money = Some(o.kind.clone()),
        "Command" => cmd = Some(o.kind.clone()),
        "DomainEvent" => event = Some(o.kind.clone()),
        "ReadModel" => read_model = Some(o.kind.clone()),
        _ => {}
    }}
    let c_money = classify_object(money.as_deref().unwrap(), "Money");
    assert!(c_money.contains(&C::Memory) && c_money.contains(&C::Schema));
