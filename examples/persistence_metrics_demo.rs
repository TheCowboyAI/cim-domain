...
use cim_domain::{
    ...
    persistence::{
        SimpleRepository, NatsSimpleRepository,
...
async fn display_metrics_summary<R: SimpleRepository<Product>>(repo: &InstrumentedRepository<Product, R>) {
...
async fn display_detailed_metrics<R: SimpleRepository<Product>>(repo: &InstrumentedRepository<Product, R>) {
    println!("\n📊 === Detailed Metrics ===\n");
    ...
    println!("  Success: {} ({:.1}%)", save_success, (save_success as f64 / save_count as f64) * 100.0);
    println!("  Errors: {} ({:.1}%)", save_errors, (save_errors as f64 / save_count as f64) * 100.0);
    ...
    println!("  Hits: {} ({:.1}%)", load_hits, (load_hits as f64 / load_count as f64) * 100.0);
    println!("  Misses: {} ({:.1}%)", load_misses, (load_misses as f64 / load_count as f64) * 100.0);
    println!("  Errors: {} ({:.1}%)", load_errors, (load_errors as f64 / load_count as f64) * 100.0);
    ...
    println!("  Found: {} ({:.1}%)", exists_true, (exists_true as f64 / exists_count as f64) * 100.0);
    println!("  Not Found: {} ({:.1}%)", exists_false, (exists_false as f64 / exists_count as f64) * 100.0);
}

async fn display_performance_analysis<R: SimpleRepository<Product>>(repo: &InstrumentedRepository<Product, R>) {
    ...
    for (op, &error_count) in &summary.errors {
        if let Some(&total) = summary.counters.get(&format!("{}.count", op)) {
            ...
            if error_rate > 0.01 {
                println!("  ⚠️  High error rate for {}: {:.1}%", op, error_rate * 100.0);
