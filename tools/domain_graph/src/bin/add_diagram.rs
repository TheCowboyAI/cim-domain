...
fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match a.as_str() {
            ...
            "--describes" => {
                if let Some(csv) = it.next() {
                    describes = csv.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
                }
            }
            "--notes" => notes = it.next(),
    ...
    if !std::path::Path::new(&svg).exists() {
        eprintln!("warning: diagram path '{}' does not exist; continuing anyway", svg);
    }
    ...
    // Ensure array exists
    if g["category"]["diagrams"].is_null() { g["category"]["diagrams"] = json!([]); }
    let mut diagrams = g["category"]["diagrams"].as_array().cloned().unwrap_or_default();

    // Replace if exists
    diagrams.retain(|d| d["id"].as_str() != Some(&id));
    let mut d = json!({ "id": id, "commutes": true, "describes": describes, "path": svg });
    if let Some(n) = notes { d["notes"] = json!(n); }
    diagrams.push(d);
    ...
    let mut files = g["metadata"]["isomorphic_to"]["string_diagrams"]["files"].as_array().cloned().unwrap_or_default();
    ...
    println!("Added diagram '{}' with {} describes", id, g["category"]["diagrams"].as_array().unwrap().last().unwrap()["describes"].as_array().unwrap().len());
    Ok(())
}
