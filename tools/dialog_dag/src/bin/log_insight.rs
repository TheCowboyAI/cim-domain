    ...
    while let Some(arg) = it.next() {
        match arg.as_str() {
            ...
            "--tags" => {
                ...
                a.tags = t.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
            }
            "--source" => a.source_event = Some(it.next().unwrap_or_default()),
            _ => {
                // Allow shorthand: file as first arg
                if a.file.is_empty() && Path::new(&arg).exists() { a.file = arg; }
            }
        }
    }
    if a.file.is_empty() { a.file = "dialog-dag.json".to_string(); }
    a
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !v.get("insights").is_some() {
        v["insights"] = json!([]);
    }
    let ins = v["insights"].as_array_mut().unwrap();
    // Dedup by id
    if let Some(existing) = ins.iter_mut().find(|x| x.get("id").and_then(|y| y.as_str()) == Some(a.id.as_str())) {
        // Update existing
        existing["summary"] = json!(a.summary);
        existing["details"] = json!(a.details);
        existing["tags"] = json!(a.tags);
        if let Some(src) = a.source_event.clone() { existing["source_event"] = json!(src); }
    } else {
        let mut entry = json!({
            "id": a.id,
            "summary": a.summary,
        ...
        if let Some(src) = a.source_event { entry["source_event"] = json!(src); }
        ins.push(entry);
    }
